    ...
    var ids = {},
        ...
        for(var i = 0; i < options.calendars; i++) {
          ...
          while(cnt < 42) {
            ...
            if (date > today) {
			 ...
			 if (date < today) {
              // current month, date in future
              data.weeks[indic].days[indic2].classname.push('datepickerPast datepickerDisabled');
            }
            ...
            if(options.date && (!$.isArray(options.date) || options.date.length > 0)) {
              if (fromUser.selected || options.date == val || $.inArray(val, options.date) > -1 || (options.mode == 'range' && val >= options.date[0] && val <= options.date[1])) {
                data.weeks[indic].days[indic2].classname.push('datepickerSelected');
				 ...
				 console.log(cal);
				 if( cal.parent('#DateUser').length > 0){
				 data.weeks[indic].days[indic2].classname.push('datepickerHolydays');
				 data.weeks[indic].days[indic2].classname.push('datepickerDisabled');
				 }
      ...
      click = function(ev) {
        ...
        if (el.is('a')) {
		  ...
		  if (el.hasClass('datepickerHolydays')) { 
            return false;
          }
          ...
          } else if (parentEl.is('td') && !parentEl.hasClass('datepickerDisabled') && !parentEl.hasClass('datepickerHolydays')) {
